...
const Checkout = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Your Donation processed successfully!");
        setTimeout(() => navigate("/thank-you"), 2000);
      } else {
        const errorData = await response.json();
        toast.error(`Your Donation processing failed: ${errorData.message}`);
      }
    } catch (error) {
      console.error("Error processing donation:", error);
      toast.error("Error processing donation. Please try again later.");
    }
  };

  const handlePayPalSuccess = async (details, data) => {
    console.log("PayPal Donation processed successful", details);
    ...
    try {
      ...
      if (response.ok) {
        toast.success("PayPal Donation verified successfully!");
        setTimeout(() => navigate("/thank-you"), 2000);
      } else {
        toast.error("Failed to verify PayPal donation. Please contact support.");
      }
    } catch (error) {
  ...
  return (
    <div className="Checkout min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8"
    style={{
      backgroundImage:
    "url('https://images.unsplash.com/photo-1603349823484-16e8823f73e6?q=80&w=1778&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')",
    }}
    >
      <ToastContainer />
      ...
      <div className="max-w-2xl w-full space-y-8">
        <Link to="/Donation">
          <div className="flex">
            ...
            <span className="text-primary font-bold ">Back to Donation</span>
          </div>
        </Link>
        ...
        <div className="mt-6 text-center">
          ...
          <img
            ...
            className="h-8 inline-block mx-2"
          />
          <p className="text-center font-extralight text-sm text-yellow-400 mt-2">
            Your donation/payment information is secure and encrypted.
          </p>
        </div>
      </div>
    </div>
